...
func main() {
	...
	log.Println("Connecting to IN endpoint...")

	epIn, err := dev.OpenEndpoint(uint8(*config), uint8(*iface), uint8(*setup), uint8(1)|uint8(usb.ENDPOINT_DIR_IN))
	if err != nil {
	...
	log.Println("Connecting to OUT endpoint...")

	epOut, err := dev.OpenEndpoint(uint8(*config), uint8(*iface), uint8(*setup), uint8(2)|uint8(usb.ENDPOINT_DIR_OUT))
	if err != nil {
		log.Fatalf("open: %s", err)
	}
	...
	cmd := []byte{0x10, 0x03, 0x00}
	num, err := epOut.Write(cmd)
	if err != nil {
